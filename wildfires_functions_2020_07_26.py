# -*- coding: utf-8 -*-
"""
Functions: Wildfires in the California

Created on Tue Jul 26 14:10:36 2020

@author: Lena Berger
"""

# ABOUT THIS FILE: 
# This file contains functions used in the various jupyter
# notebooks. 

# FUNCITON FOR BIGQUERY
# This is taken from GitHub. 
...
# FUNCTIONS TO CALCULATE DISTANCE BETWEEN POINTS
...
def closest(data, this_point):
    ...
    for index, row in data.iterrows():
        dist.append([index, distance(this_point, row)])
    
    dist = pd.DataFrame(dist, columns = ["index", "distance"])
    return(dist)
    
    
# FUNCTION TO REPLACE MISSING
def replace_missing(data, variable, missing_value): 
    data.loc[data[variable] == missing_value, variable] = np.nan
    return data

# SPEARMAN'S TEST
def spearman_test(title, a, b, data): 
    dat = data[[a, b]].dropna()
    spearman_r = spearmanr(dat[a], dat[b])
    print("-------------------------------------------------------------------------------------")
    print(title)
    print("Spearman's r is " + str(spearman_r[0]) + ". The p-values is " + str(spearman_r[1]) + ".")
    print("-------------------------------------------------------------------------------------")
    
# FIRE SIZE DICTIONARY
# Prepare the dictionary
fire_size_dict = {"A": "<= 0.25 acres", 
              "B": "0.26-9.9 acres", 
              "C": "10.0-99.9 acres", 
              "D": "100-299 acres", 
              "E": "300-999 acres", 
